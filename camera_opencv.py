...
def follow_lane(frame2, curr_steering_angle):
    # Main entry point of the lane follower
    frame = preprocessImage(frame2)
    lane_lines, frame = detect_lane(frame, frame2)
    final_frame, curr_steering_angle = steer(
        frame, lane_lines, curr_steering_angle)
...
def detect_lane(frame, frame2):
    ...
    line_segment_image = display_lines(frame2, line_segments)

    lane_lines = average_slope_intercept(frame, line_segments)
    lane_lines_image = display_lines(frame2, lane_lines)
...
def test_video(video_file):
    ...
    try:
        ...
        while cap.isOpened():
            _, frame = cap.read()
            print('frame %s' % i)
            combo_image = follow_lane(frame, 90)
